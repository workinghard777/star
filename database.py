...
async def get_awaiting_orders(currency: str | None = None):
    async with aiosqlite.connect(cfg.db_path) as db:
        db.row_factory = aiosqlite.Row
        if currency:
            cur = await db.execute(
                "SELECT * FROM orders WHERE status='awaiting_payment' AND currency=?", (currency,)
            )
        else:
            cur = await db.execute("SELECT * FROM orders WHERE status='awaiting_payment'")
